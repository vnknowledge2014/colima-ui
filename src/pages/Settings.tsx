...
export default function Settings({ systemInfo }: SettingsProps) {
  ...
  const handlePrune = async () => {
    if (!confirm("Remove all unused Docker data (stopped containers, unused networks, dangling images, build cache)?")) return;
    setPruning(true);
    try {
      await dockerApi.systemPrune();
      notify("success", "System pruned successfully");
      fetchDiskUsage();
    } catch (e) {
