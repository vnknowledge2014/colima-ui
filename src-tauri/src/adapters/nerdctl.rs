...
impl NerdctlAdapter {
    ...
    async fn cli_output(&self, args: Vec<String>) -> Result<std::process::Output, AdapterError> {
        let result = tokio::time::timeout(
            ...
            tokio::task::spawn_blocking(move || {
                crate::commands::runtime::get_runtime_cmd()
                    ...
                    .output()
                    .map_err(|e| AdapterError::new("nerdctl", format!("Failed to run: {}", e)))
            }),
        )
        .await;
        ...
        match result {
            Ok(Ok(output)) => output,
            Ok(Err(e)) => Err(AdapterError::new("nerdctl", format!("Task error: {}", e))),
            Err(_) => Err(AdapterError::new("nerdctl", "Command timed out (daemon may be unresponsive)")),
        }
    }
...
#[async_trait]
impl ContainerRuntime for NerdctlAdapter {
    fn name(&self) -> &str {
        "nerdctl"
    }
    ...
    async fn list_containers(&self, all: bool) -> Result<Vec<Container>, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", String::from_utf8_lossy(&output.stderr).to_string()));
        }
    ...
    async fn start_container(&self, id: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("start failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn stop_container(&self, id: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("stop failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn restart_container(&self, id: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("restart failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn remove_container(&self, id: &str, force: bool) -> Result<String, AdapterError> {
        ...
        let output = self.cli_output(args).await?;
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("rm failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn container_logs(&self, id: &str, lines: u32) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("logs failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
        let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    async fn container_stats(&self, id: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("stats failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn run_container(&self, config: RunConfig) -> Result<String, AdapterError> {
        ...
        let output = self.cli_output(args).await?;
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("run failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn exec(&self, id: &str, command: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            if !stderr.is_empty() {
                return Err(AdapterError::new("nerdctl", format!("exec failed: {}", stderr)));
            }
            return Err(AdapterError::new("nerdctl", "exec failed with no output"));
        }
        Ok(String::from_utf8_lossy(&output.stdout).to_string())
    }
    ...
    async fn inspect_container(&self, id: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("inspect failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
        Ok(String::from_utf8_lossy(&output.stdout).to_string())
    }
    ...
    async fn pull_image(&self, name: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("pull failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
        Ok(String::from_utf8_lossy(&output.stdout).to_string())
    }
    ...
    async fn remove_image(&self, id: &str, force: bool) -> Result<String, AdapterError> {
        ...
        let output = self.cli_output(args).await?;
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("rmi failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn prune_images(&self) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("prune failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
        Ok(String::from_utf8_lossy(&output.stdout).to_string())
    }
    ...
    async fn create_volume(&self, name: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("volume create failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn remove_volume(&self, name: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("volume rm failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn create_network(&self, name: &str, driver: &str) -> Result<String, AdapterError> {
        ...
        let output = self.cli_output(args).await?;
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("network create failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn remove_network(&self, name: &str) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("network rm failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
    ...
    async fn system_df(&self) -> Result<String, AdapterError> {
        ...
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("system df failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
        Ok(String::from_utf8_lossy(&output.stdout).to_string())
    }
    ...
    async fn system_prune(&self, all: bool) -> Result<String, AdapterError> {
        ...
        let output = self.cli_output(args).await?;
        if !output.status.success() {
            return Err(AdapterError::new("nerdctl", format!("system prune failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
        Ok(String::from_utf8_lossy(&output.stdout).to_string())
    }
        ...
        match cmd.status() {
            ...
            Err(e) => {
                if e.kind() == std::io::ErrorKind::NotFound {
                    Err(AdapterError::new("nerdctl", "Command not found. Is docker/podman installed?"))
                } else {
                    Err(AdapterError::new("nerdctl", format!("Failed to execute passthrough: {}", e)))
                }
            }
        }
    }
