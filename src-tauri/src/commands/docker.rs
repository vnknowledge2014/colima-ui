...
/// List all Docker containers via Docker CLI.
/// Always fetches fresh data — no caching, no Bollard dependency.
#[tauri::command]
pub async fn list_containers(all: bool) -> Result<Vec<serde_json::Value>, String> {
...
pub async fn list_images() -> Result<Vec<serde_json::Value>, String> {
    let output = docker_cmd()
        .args(["images", "--format", "json", "--no-trunc"])
        .output()
